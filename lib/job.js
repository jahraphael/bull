...
Job.fromData = function(queue, jobId, data){
  ...
  try {
    ...
    debuglog(jobId + ' has corrupted data: ' + data.data, e);
    parsedData = {};
  }
  ...
  try {
    ...
    debuglog(jobId + ' has corrupted opts: ' + data.opts, e);
    parsedOpts = {};
  }

  var job = new Job(queue, parsedData, parsedOpts);
  job.jobId = jobId;
  ...
  try{
    ...
    debuglog(jobId + ' corrupted returnvalue: ' + data.returnvalue, e);
  }
  return job;
};
