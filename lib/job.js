...
Job.fromId = function(queue, jobId){
  ...
  return queue.client.hgetall(queue.toKey(jobId)).then(function(jobData){
    if(jobData.data === undefined || jobData.opts === undefined) {
      console.log('job', queue.toKey(jobId), 'is missing "data" or "opts" field -- removing job', jobData);
    ...
    return _this.isCompleted().then(function(completed){
      if(!completed){
        return _this.isFailed().then(function(failed){
          if(failed){
            console.log('calling Job.formId for to get failedREason');
            return Job.fromId(_this.queue, _this.jobId, 'failedReason').then(function(data){
              console.log('data is', data);
              if (data === null){
                _this.remove().finally(function () {
                  reject('Missing data in Job.finished');
                });
                return true;
              }
...
Job.fromData = function(queue, jobId, data){
  try {
    ...
    console.log('!@# caught error', e, 'for job', queue.toKey(jobId));
  }
};
